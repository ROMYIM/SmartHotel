...
Component({
  ...
  methods: {
    ...
    selectDay(e) {
      ...
      let datetime = new Date(canlender.year + "/" + month + "/" + date);
      console.log('当前时间:' + datetime.getTime());
      let startDate = null;
      ...
      if (this.properties.target == 'enter') {
        startDate = new Date();
      } else if (this.properties.target == 'leave') {
        startDate.setDate(startDate.getDate() + 1)
      }
      startDate = new Date(formatTime(startDate).fullDate.replace(/-/g, '/'));
    ...
    packup() {
      ...
      if (this.data.isOpen) {
        ...
        let selectDate = new Date(_date.replace(/-/g, '/'));
        let startDate = null;
        if (!this.data.startDate) {
          startDate = new Date();
        } else {
